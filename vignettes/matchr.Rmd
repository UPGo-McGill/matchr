...
Here we demonstrate `match_images` being run on a single vector of internet images, using the `test_urls` data object included in matchr:
...
```{r match_images}
matches <- match_images(test_urls, compare = FALSE)
matches
```
...
The "index" field refers to the position of the matches in the correlation matrices which are generated as part of the matching process, but since `match_images` skips a number of intermediate outputs which are normally created by the matchr functions which it wraps, this field isn't meaningful. The next two fields ("x_sig" and "y_sig") contain the `matchr_signature` image signature vectors for the images which were matched. (In an GUI environment such as RStudio, these are displayed as colour patterns, although in this vignette they are monochrome.) These vectors contain the image signatures used for matching, alongside other metadata from the images such as resolution, colour channels, and file path. The final two fields ("correlation" and "match") give, respectively, the Pearson correlation coefficient between the greyscale image signatures of the two matching images and an additional confirmation based on the colour image signatures (which can be "match", "likely match", "possible match" or "no match"). If `match_images` is run with its default `compare = TRUE` setting, using the interactive image comparison Shiny app to manually confirm the results of the matching process will add `confirmed` and `highlight` fields to the output data frame, which respectively identify image pairs whose status ("match" or "no match") were visually confirmed and image pairs flagged for further follow up.
...
For small image sets such as the `test_urls` example, we can read the images directly into memory using `load_image`. 
...
In a GUI such as RStudio, `matchr_signature` vectors print as a series of colour blocks to allow for easy visual inspection of the underlying data; in this vignette the class falls back on a simpler presentation of the average values for each of the greyscale, red, green and blue colour bands. This simple presentation abstracts away a more detailed internal data structure: by default, `create_signature` generates average values for 20 horizontal bands and 20 vertical bands in greyscale, red, green and blue. This yields a numeric vector of length 160:

```{r get_signature}
get_signature(sig[1])
```

While this is a drastic simplification of the data present in an image, it is sufficient to identify distinctive patterns which are highly unlikely to be exactly shared by other images. Using the `plot` method for `matchr_signature` vectors, the signature can be visualized as follows:

```{r col_img, fig.height = 6, fig.width = 7}
plot(sig[5])
```

The `bands` argument to `create_signature` controls how many horizontal and vertical bands will be used to generate an image signature. The default is 20, which was determined through testing to represent a good balance between accuracy and performance. In general, higher values will yield signatures which are less likely to be matched to other signatures, which will avoid false positives but also potentially introduce false negatives. Matching signatures with higher `bands` values will also take more computing resources, in a roughly linear relationship. (E.g. comparing signatures with `bands = 40` will take approximately twice as long as comparing signatures with `bands = 20`.)

### Removing black bars

A relatively common phenomenon with user-uploaded photographs is horizontal black bars above and below an image. These occur when users take a screenshot of a landscape-oriented photograph and upload the result. These images pose a difficulty for matchr's image signature strategy, because they lead to signatures dominated by black values:
...
Because matchr was motivated by the need to compare large numbers of user-uploaded photographs across different web services, it has the ability to detect and remove horizontal black bars, using the `rm_black_bars` argument to `create_signature`, which is TRUE by default. When `rm_black_bars = TRUE`, the underlying image is not modified upon import, but its image signature is only generated from the portion of the image which does not have rows of entirely black pixels:
...
sig_1 <- get_signature(sig)[[1]][1:40] 
sig_2 <- get_signature(sig)[[2]][1:40]
sig_5 <- get_signature(sig)[[5]][1:40]
