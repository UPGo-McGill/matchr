...
#'
#' \code{integrate_changes_2} takes the results of manual image comparison using
...
integrate_changes_2 <- function(result, change_table) {
  ...
  } else output[!is.na(output$highlight),]$highlight <- 
    output[!is.na(output$highlight),]$new_highlight
